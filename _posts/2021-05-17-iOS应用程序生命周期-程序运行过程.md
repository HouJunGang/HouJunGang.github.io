...
在入口`main函数`中将`Appdelegate`作为了整个应用的代理委托用于处理**App生命周期**和**UI生命周期**

## SceneDelegate - iOS 13新增Api

在`iOS 13`以后，`SceneDelegate`接管了原本由`Appdelegate`处理的**UI生命周期**

`Appdelegate`处理**App生命周期**和新的**Scene Session生命周期**

### 适配iOS 13以下版本，iOS 13以上也可以删掉使用老的办法

1. **删除**`SceneDelegate`文件
2. **删除**`info.plist`里的`Application Scene Manifest`项
3. **还原**`Appdelegate`里的window属性
